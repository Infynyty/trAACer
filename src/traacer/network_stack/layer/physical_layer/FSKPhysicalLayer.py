...
async def main() -> None:
    source = string_source(["Hello", "World"])
    ...
    symbol_stream = bits_to_fsk_symbols.process(bit_stream)
    ...
    find_packets = FindPackets(
        preamble=preamble,
        packet_num_samples=(5 * 8 // config.bits_per_symbol) * config.samples_per_symbol,
        threshold=0.06,
    )
    ...
    rx_byte_stream = bits_to_bytes.process(rx_bit_stream)
