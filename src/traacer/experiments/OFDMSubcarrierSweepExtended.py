...
PLOT_OUTPUT_PATH = Path("measurements/ofdm-subcarrier-ber.svg")
SIGNAL_SPACE_PLOT_OUTPUT_PATH = Path("measurements/ofdm-subcarrier-signal-space.svg")
...
class OFDMSignalSpacePlot(Stage[QAMSymbolBlock, QAMSymbolBlock]):
    ...
    def _update_plot(self, block: QAMSymbolBlock, count: int) -> None:
        ...
        if self._plotted_counts == set(self.positive_subcarrier_counts):
            ...
            print(
                f"Signal-space plot written to "
                f"{SIGNAL_SPACE_PLOT_OUTPUT_PATH}"
            )
